...
def L_model_softmax_forward_test_case_2hidden():
    np.random.seed(6)
...
def softmax_compute_cost_test_case():  #(4,3) - 4 categories, 3 examples,
    np.random.seed(3)
    Y = np.asarray([[0,0,1], [0,1,0], [1,0,0], [0,0,0]])
    aL = u10_softmax(np.random.randn(4, 3))[0]
    return Y, aL
...
def L_model_softmax_backward_test_case():

    np.random.seed(5)
    Y = np.asarray([[0,1,0], [0,1,0], [1,0,0], [0,0,0]])
    AL = u10_softmax(np.random.randn(4, 3))[0]
